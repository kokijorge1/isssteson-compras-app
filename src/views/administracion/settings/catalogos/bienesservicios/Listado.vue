      ...
      <v-card-text>
        ...
        <v-data-table
          ...
          <template slot="items" slot-scope="props">
            <td>{{ (props.index + 1) }}</td>
            <td>{{ props.item.partida.clave }}</td>
            <td>{{ props.item.clave_interna }}</td>
            <td>{{ props.item.claves_internas_concatenadas }}</td>
            <td>
              <strong>
                {{ props.item.cabms }}
              </strong>
            ...
            <td>
              <fecha-calendario
                :fecha="new Date(props.item.updated_at)">
              </fecha-calendario>
            </td>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Clave Médica', sortable: true, value: 'clave_interna' },
        { text: 'Claves Internas', sortable: false, value: 'claves_unidades' },
        { text: 'CABMS', sortable: true, value: 'cabms' },
        { text: 'Nombre', sortable: true, value: 'nombre' },
        { text: 'Unidad M.', sortable: true, value: 'unidad' },
        { text: 'Presentación', sortable: true, value: 'presentacion' },
        { text: 'Modificado', sortable: true, value: 'updated_at' },
        { text: '', sortable: false, value: '' },
      ],
    };
  },
  methods: {
    ...mapActions('settings/catalogos', ['borrarArticulo']),
    onEditar(id) {
      this.idEditar = id;
      this.editar = true;
    },
  ...
  computed: {
    ...mapGetters({
      articulos: 'settings/catalogos/articulos',
    }),
    listado() {
