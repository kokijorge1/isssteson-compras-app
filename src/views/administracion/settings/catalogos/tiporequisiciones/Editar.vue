...
export default {
  ...
  methods: {
    ...mapActions('settings/catalogos', ['actulizarTipoRequisicion']),
    guardar() {
      this.enviando = true;
      this.actulizarTipoRequisicion({
        id: this.id,
        tipoRequisicion: this.tipoRequisicion,
      })
        .then(() => {
  ...
  computed: {
    ...mapGetters({
      ...
      tipoRequisicionId: 'settings/catalogos/tipoRequisicion',
    }),
    listado() {
      return this.partidasGenericas;
    },
    getTipoRequisicion() {
      return this.tipoRequisicionId(this.id);
    },
    ordenarChips() {
  ...
  watch: {
    id: {
      handler(value) {
        if (!value) return;
        this.tipoRequisicion.descripcion = this.getTipoRequisicion.descripcion;
        this.partidasSelected = this.getTipoRequisicion.tipo_partidas_genericas
          .map(item => item.partida_generica);
      },
    },
    partidasSelected: {
